...
이 프로젝트는 마이그레이션 도구 없이 부팅 시점에 스키마를 맞춥니다. 자동으로 따라오는
변경과 그렇지 않은 변경은 아래와 같습니다.

| 변경                   | 자동 반영 | 방법                                                                                          |
| ---------------------- | :-------: | --------------------------------------------------------------------------------------------- |
| 새 테이블              |     O     | `sequelize.sync()` (`alter:false`)                                                            |
| 새 컬럼                |     O     | 부팅 시 컬럼 보강 — 모델과 실제 테이블을 비교해 `ADD COLUMN`                                  |
| 새 인덱스(새 테이블)   |     O     | `sync()` 가 테이블을 만들 때 함께 생성                                                        |
| 새 인덱스(기존 테이블) |   **X**   | 모델에 적어도 생기지 않음(`alter:false`) — `scripts/add-indexes.ts` 목록에 추가(부팅 시 실행) |
| ENUM → 문자열 넓히기   |     O     | 부팅 시 자동(알림 종류·테마). MySQL/PostgreSQL 만 해당, SQLite 는 원래 TEXT                   |
| 컬럼 타입·제약 변경    |   **X**   | 직접 `ALTER TABLE`                                                                            |
| 컬럼 이름 변경·삭제    |   **X**   | 직접 `ALTER TABLE` (모델에서 지워도 컬럼은 남음)                                              |

> 컬럼 보강은 `sync()` 보다 **먼저** 실행됩니다. 새 컬럼에 인덱스를 함께 추가할 때
> 컬럼이 없는 상태에서 인덱스를 만들면 부팅이 실패하기 때문입니다.

ENUM 넓히기가 실패해도 기동은 계속되고, 직접 실행할 SQL 이 로그에 남습니다.
해당 기능만 동작하지 않는 상태가 될 수 있으므로 배포 후 서버 로그를 확인하세요.
