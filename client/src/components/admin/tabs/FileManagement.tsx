...
export const FileManagement = React.memo(() => {
  ...
  const fetchFiles = () => queryClient.invalidateQueries({ queryKey: adminKeys.files.all });
  ...
  const handleDelete = async (file: FileItem) => {
    ...
    try {
      await deleteAdminFile(file.type, file.filename);
      toast.success('파일이 삭제되었습니다.');
      setConfirmDelete(null);
      // 현재 페이지의 마지막 파일을 삭제했고 1페이지가 아니면 이전 페이지로(빈 페이지 방지).
      if (files.length === 1 && page > 1) setPage(p => p - 1);
      // 삭제된 파일이 어느 페이지 캐시에 남아있을지 모르므로 목록 전체를 무효화한다.
      await fetchFiles();
    } catch (err: unknown) {
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-700">
                <th className="admin-th">파일명</th>
                <th className="admin-th hidden sm:table-cell">유형</th>
                <th className="admin-th hidden sm:table-cell">크기</th>
                <th className="admin-th hidden md:table-cell">업로드 일시</th>
            ...
            <tbody>
              {files.map(file => (
                ...
                >
                  ...
                  <td className="admin-td hidden sm:table-cell">{formatBytes(file.size)}</td>
                  <td className="admin-td hidden md:table-cell text-xs">
