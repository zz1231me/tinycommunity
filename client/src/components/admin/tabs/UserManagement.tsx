        ...
        return (
          <div className="flex items-center justify-end gap-1.5">
            ...
            <button
              ...
              className="px-3 py-1.5 text-xs rounded-md border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-amber-50 hover:border-amber-300 hover:text-amber-600 dark:hover:bg-amber-900/20 transition-colors"
            >
              비활성화
            </button>
            <button
              onClick={() => setActivityModal({ userId: u.id, userName: u.name })}
              className="px-3 py-1.5 text-xs rounded-md border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-secondary-50 hover:border-secondary-300 hover:text-secondary-600 dark:hover:bg-secondary-900/20 transition-colors"
            >
              활동 내역
            </button>
            <button
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="admin-th">아이디</th>
                  <th className="admin-th">이름</th>
                  <th className="admin-th">신청일</th>
                  <th className="admin-th text-right">작업</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {pendingUsers.map(user => (
                  <tr key={user.id} className="hover:bg-amber-50 dark:hover:bg-amber-900/10">
                    <td className="admin-td font-mono text-slate-900 dark:text-slate-100">
                      {user.id}
                    </td>
                    <td className="admin-td font-medium text-slate-900 dark:text-slate-100">
                      {user.name}
                    </td>
                    <td className="admin-td text-xs">{formatDateTime(user.createdAt)}</td>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="admin-th">아이디</th>
                  <th className="admin-th">이름</th>
                  <th className="admin-th">마지막 로그인</th>
                  <th className="admin-th text-right">작업</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              ...
              {table.getHeaderGroups().map(headerGroup => (
                ...
                >
                  {headerGroup.headers.map(header => {
                    ...
                    return (
                      <th key={header.id} className={`admin-th ${isActions ? 'text-right' : ''}`}>
                        {canSort ? (
                          <button
                          ...
                          >
                            ...
                            <span
                              className={`text-2xs ${sorted ? 'text-primary-500' : 'text-slate-300 dark:text-slate-600'}`}
                            >
                              {sorted ? (sorted === 'asc' ? '▲' : '▼') : '↕'}
                            </span>
                          </button>
                ...
                table.getRowModel().rows.map(row => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    {row.getVisibleCells().map(cell => (
                      <td
                        key={cell.id}
                        className={`admin-td ${cell.column.id === 'actions' ? 'text-right' : ''} ${cell.column.id === 'lastSeen' ? 'whitespace-nowrap' : ''} ${cell.column.id === 'createdAt' ? 'whitespace-nowrap text-slate-500 dark:text-slate-400' : ''}`}
                      >
          ...
          <div className="flex items-center justify-between px-3 py-3 border-t border-slate-100 dark:border-slate-700/60">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="px-3 py-1 text-xs rounded-md border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
                이전
              </button>
              <button
        ...
        {showDeleted && (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="admin-th">익명화 ID</th>
                  <th className="admin-th">이름</th>
                  <th className="admin-th">삭제일</th>
                  <th className="admin-th text-right">작업</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {deletedUsers.length === 0 ? (
                  <tr>
                    <td colSpan={4}>
                      <ListState>삭제된 계정이 없습니다.</ListState>
                    </td>
                  </tr>
                ) : (
                  deletedUsers.map(user => (
                    <tr key={user.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      <td className="admin-td font-mono">
                        {user.anonymizedName || `삭제된계정_${user.id}`}
                      </td>
                      <td className="admin-td">{user.name}</td>
                      <td className="admin-td text-xs text-slate-400">
