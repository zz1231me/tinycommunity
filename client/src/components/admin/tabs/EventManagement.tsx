              ...
              <table className="min-w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    <th className="admin-th">역할</th>
                    <th className="admin-th text-center">생성</th>
                    <th className="admin-th text-center">조회</th>
                    <th className="admin-th text-center">타인 수정</th>
                    <th className="admin-th text-center">타인 삭제</th>
                  </tr>
                </thead>
                  ...
                  {eventPermissions.map(p => (
                    <tr key={p.roleId} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      {(['canCreate', 'canRead', 'canUpdate', 'canDelete'] as const).map(key => (
                        <td key={key} className="admin-td text-center">
                          <input
                            type="checkbox"
                            checked={p[key]}
              ...
              <tr className="border-b border-slate-200 dark:border-slate-700">
                <th className="admin-th">
                  ...
                  >
                    제목
                    <span
                      className={`text-2xs ${sortKey === 'title' ? 'text-primary-500' : 'text-slate-300 dark:text-slate-600'}`}
                    >
                      {sortKey === 'title' ? (sortDir === 'asc' ? '▲' : '▼') : '↕'}
                    </span>
                  </button>
                </th>
                <th className="admin-th">분류</th>
                <th className="admin-th">작성자</th>
                <th className="admin-th">
                  <button
                  ...
                  >
                    기간
                    <span
                      className={`text-2xs ${sortKey === 'start' ? 'text-primary-500' : 'text-slate-300 dark:text-slate-600'}`}
                    >
              ...
              {sortedEvents.length === 0 ? (
                ...
                pagedEvents.map(event => (
                  <tr key={event.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50">
                    <td className="admin-td font-medium text-slate-900 dark:text-slate-100 min-w-[160px]">
                      {editingId === event.id ? (
                        <input
                          type="text"
                          value={editTitle}
