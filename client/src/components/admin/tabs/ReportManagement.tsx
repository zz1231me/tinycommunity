...
export const ReportManagement = React.memo(() => {
  ...
  const [listQuery, statsQuery] = useQueries({
    queries: [
      {
        queryKey: adminKeys.reports.list({ statusFilter, typeFilter, page }),
        queryFn: () =>
          getReports({
      ...
      // 통계 실패는 조용히 무시하던 기존 동작 유지 — 목록과 독립된 쿼리라 서로 영향 없음
      { queryKey: adminKeys.reports.stats, queryFn: getReportStats },
    ],
  });

  const reports: Report[] = listQuery.data?.items ?? [];
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      setReviewNote('');
      // 현재 페이지의 마지막 항목을 처리했고 1페이지가 아니면 이전 페이지로 이동(빈 페이지 방지).
      if (reports.length === 1 && page > 1) setPage(p => p - 1);
      // 처리된 신고가 어느 페이지 캐시에 남아있을지 모르므로 목록 전체를 무효화한다.
      await fetchReports();
      await fetchStats();
    } catch (err: unknown) {
