...
export function BookmarkManagement() {
  ...
  const loadBookmarks = () => queryClient.invalidateQueries({ queryKey: adminKeys.bookmarks.all });

  // 순서 변경은 낙관적 반영 — 연속 클릭이 갱신된 순서를 기준으로 계산되도록.
  const reorderMutation = useMutation({
    mutationFn: (ordered: Bookmark[]) =>
      reorderBookmarks(ordered.map((b, i) => ({ id: b.id, order: i }))),
    onMutate: async (ordered: Bookmark[]) => {
      await queryClient.cancelQueries({ queryKey: adminKeys.bookmarks.all });
      const previous = queryClient.getQueryData<Bookmark[]>(adminKeys.bookmarks.all);
      queryClient.setQueryData(adminKeys.bookmarks.all, ordered);
      return { previous };
    },
    onError: (_e, _v, ctx) => {
      if (ctx?.previous) queryClient.setQueryData(adminKeys.bookmarks.all, ctx.previous);
