...
const BoardManagerManagement = () => {
  ...
  const fetchBoards = () =>
    queryClient.invalidateQueries({ queryKey: adminKeys.boardManagers.all });
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  {!selectedBoard.boardManagers?.length ? (
                    <ListState>지정된 담당자가 없습니다</ListState>
                  ) : (
                    selectedBoard.boardManagers.map(record => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-xs bg-primary-100 dark:bg-primary-900/30 flex items-center justify-center text-primary-600 dark:text-primary-400 font-semibold text-sm">
                            {record.user?.name?.[0] ?? '?'}
                          </div>
                          <div>
