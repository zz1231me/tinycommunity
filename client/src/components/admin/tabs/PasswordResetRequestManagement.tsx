...
export const PasswordResetRequestManagement = () => {
  ...
  const {
    ...
    isFetching,
    refetch,
  } = useQuery<PasswordResetRequestItem[]>({
    queryKey: adminKeys.passwordResets.all,
    queryFn: fetchPasswordResetRequests,
  });

  const dismissMutation = useMutation({
    mutationFn: (id: string) => dismissPasswordResetRequest(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: adminKeys.passwordResets.all }),
  });
  const busyId = dismissMutation.isPending ? dismissMutation.variables : null;
  ...
  const copyCode = async (code: string) => {
    if (await copyText(code)) toast.success('인증번호를 복사했습니다.');
    else toast.error('복사에 실패했습니다. 직접 확인해 전달하세요.');
  };

  const dismiss = async (req: PasswordResetRequestItem) => {
    if (busyId) return;
    try {
      await dismissMutation.mutateAsync(req.id);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        {requests.length === 0 ? (
          <ListState size="roomy">진행 중인 초기화 요청이 없습니다.</ListState>
        ) : (
            ...
            {requests.map(req => {
              ...
              return (
                <div key={req.id} className="card p-4">
                  ...
                  {/* 인증번호 */}
                  <div className="mt-3 flex items-center gap-2 rounded-xl bg-slate-50 dark:bg-slate-900/40 px-3 py-3">
