...
export const TagSelector: React.FC<TagSelectorProps> = ({ selectedTags, onChange, boardId }) => {
  ...
  const featureEnabled = useFeature('post.tags');
  ...
  return (
    <div className="space-y-2">
      ...
      {allTags.length === 0 ? (
        <p className="text-xs text-slate-400 py-1">등록된 태그가 없습니다.</p>
      ) : shown.length === 0 ? (
        <p className="text-xs text-slate-400 py-1">검색 결과가 없습니다.</p>
      ) : (
          ...
          {shown.map(tag => {
            ...
            return (
              <button
                ...
                onClick={() => toggle(tag)}
                aria-pressed={active}
                className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border transition-all active:scale-[0.97] ${
                  active
      ...
      {selectedTags.length > 0 && (
        <p className="text-xs text-slate-400">{selectedTags.length}개 선택됨</p>
      )}
    </div>
  );
};
