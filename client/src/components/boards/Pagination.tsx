...
export const Pagination = memo<PaginationProps>(({ pagination, currentPage, onPageChange }) => {
  ...
  const navBtn = (
    ...
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className="w-11 h-11 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg text-slate-400
...
const Ellipsis = () => (
  <span className="w-8 h-8 flex items-center justify-center text-sm text-slate-400 select-none">
    …
  </span>
);
