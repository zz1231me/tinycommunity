...
export function TempShareModal({ open, onClose }: Props) {
  ...
  const copy = async () => {
    if (await copyText(shareUrl)) toast.success('링크를 복사했습니다.');
      ...
      >
        ...
        <div className="flex items-start gap-3 px-5 pt-5 pb-4">
          ...
          <button
            onClick={handleClose}
            aria-label="닫기"
            className="tap-target -mr-1 flex-shrink-0 rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-300"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          {uploading ? (
            ...
            <>
              ...
              >
                ...
                <p className="text-xs text-slate-400">최대 {maxMb}MB · 파일 형식 제한 없음</p>
              </div>
              <input
                ref={inputRef}
                type="file"
