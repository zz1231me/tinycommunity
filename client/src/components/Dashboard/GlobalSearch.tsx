...
export function GlobalSearch() {
  ...
  const featureEnabled = useFeature('search.global');
  ...
  return (
    <>
      {/* min-w-0: flex 아이템의 기본 min-width:auto 는 내용(입력창)보다 작아지지 않아,
          좁은 화면에서 헤더가 뷰포트를 넘겨 가로 스크롤을 만든다. */}
      <div className="relative flex-1 min-w-0 max-w-2xl mx-1 sm:mx-4" ref={searchRef}>
        {/* 검색 버튼 (닫힌 상태) */}
        {!isOpen && (
          <button
            onClick={handleOpen}
          ...
          >
            ...
            {/* 글자도 줄어들 수 있어야 한다 — 예전에는 이 span 이 고유 폭을 지켜서
                좁은 화면에서 버튼 밖으로 삐져나가 오른쪽 알림·계정 아이콘과 겹쳤다. */}
            <span className="min-w-0 truncate text-sm text-slate-500 dark:text-slate-400">
              전체검색
            </span>
            <kbd className="hidden sm:inline-flex items-center gap-1 ml-auto px-2 py-1 text-xs font-mono text-slate-400 bg-white dark:bg-slate-800 rounded">
              ⌘K
            </kbd>
          </button>
        )}
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="relative p-4">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  aria-label="검색 닫기"
                  className="px-3 py-1.5 text-xs font-medium text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors flex-shrink-0"
                >
                  ESC
                </button>
              </div>
            ...
            {/* 필터 버튼 — 결과 위 */}
            {results.length > 0 && (
              <div className="px-4 py-3 flex items-center gap-2 flex-wrap border-b border-slate-100 dark:border-slate-700/50">
                <button
                  ...
                  .map(type => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1 text-xs font-medium rounded-full transition-all ${colorMap[type]}`}
                      >
                ...
                <div className="py-2">
                  ...
                  <div className="space-y-0.5">
                    {history.map(query => (
                      <div
                        key={query}
                        className="flex items-center gap-3 px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors group"
                      >
                  ...
                  {orderedGroups.map(type => {
                    ...
                    return (
                      <div key={type} className="mb-3 last:mb-0">
                        {/* 섹션 헤더 */}
                        <div className="px-4 py-3 bg-gradient-to-r from-slate-50/50 to-transparent dark:from-slate-900/50 dark:to-transparent">
                          <div className="flex items-center gap-3">
                            <div
