...
describe('선택 반영', () => {
  ...
  // 키 입력은 목록이 보인다고 바로 먹지 않는다.
  //
  // 목록은 렌더 커밋 시점에 DOM 에 들어가지만, document 의 keydown 리스너는 그 뒤
  // effect 에서 붙는다. 그 사이에 누른 키는 아무 데도 닿지 않고 사라진다.
  // 평소에는 findBy* 가 act 로 감싸 effect 를 흘려보내 통과하지만, 부하가 걸려
  // 타이머가 밀리면 DOM 만 먼저 보이는 창이 열린다 — CI 에서 이 테스트가
  // insertedText=null 로 실패한 적이 있다.
  //
  // 그래서 '한 번 누르고 기다린다' 가 아니라 '먹을 때까지 누른다' 로 쓴다.
  // 성공하면 close() 가 리스너를 떼므로 추가 입력은 무해하다.
  ...
  it('↓ 로 이동한 뒤 Enter 하면 두 번째 항목이 들어간다', async () => {
    ...
    // 위 Enter 테스트와 같은 이유로 '먹을 때까지 누른다'.
    // 누른 직후 곧바로 단언하므로, 먹는 순간 멈춰 인덱스가 더 넘어가지 않는다.
    await waitFor(() => {
      fireEvent.keyDown(document, { key: 'ArrowDown' });
      expect(screen.getByRole('option', { selected: true })).toHaveTextContent('바비');
    });
    await waitFor(() => {
      fireEvent.keyDown(document, { key: 'Enter' });
      expect(rec.insertedText).toBe('@bobby ');
    });
  });
  ...
  it('Escape 로 닫는다', async () => {
    ...
    // 같은 이유 — 리스너가 붙기 전의 Escape 는 사라진다
    await waitFor(() => {
