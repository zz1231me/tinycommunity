...
api.interceptors.response.use(
  ...
  async error => {
    ...
    // 419 = "다시 발급받으면 되는 상태". 서버가 그렇게 구분해서 준다:
    //   - access 토큰이 만료됐거나, 없더라도 refresh 토큰이 남아 있으면 → 419
    //   - 정말 끝난 세션(둘 다 없음, 무효화된 토큰) → 401
    // 상태 코드만 본다. 응답 메시지 문구로 판단하면 문구가 바뀌는 순간 세션이 조용히 끊긴다.
    //
    // 여기에 isAuthEndpoint 를 걸면 안 된다. 앱을 열 때 먼저 부르는 /auth/me 가 419 를
    // 받는 자리라, 제외하면 새로고침할 때마다 갱신 없이 로그아웃된다.
    // isAuthEndpoint 는 갱신까지 실패했을 때 로그인 화면으로 보낼지에만 쓴다.
    ...
    // 401 상태 코드: 인증 실패 (위에서 갱신을 시도했다가 실패한 것도 여기로 온다)
