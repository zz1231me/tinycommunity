// server/src/services/thumbnail.service.ts
// 첨부 이미지 썸네일 — 요청 시점에 생성하고 디스크에 캐시한다.
//
// 업로드 시점이 아니라 지연 생성하는 이유:
//  - 이미 올라와 있는 첨부에도 마이그레이션 없이 적용된다
//  - 아무도 미리보기하지 않는 이미지에 CPU·디스크를 쓰지 않는다
//  - 업로드 응답이 sharp 처리만큼 느려지지 않는다
...
/** 썸네일 최대 변 길이(px). 목록·미리보기 용도라 이 정도면 충분하다. */
...
/**
 * 원본 첨부 경로로부터 썸네일을 만들고(없으면) 그 경로를 돌려준다.
 * 이미지가 아니거나 손상돼 sharp 가 처리하지 못하면 null.
 *
 * 출력은 항상 sharp 가 재인코딩한 JPEG 다. 원본 바이트를 그대로 내보내지 않으므로
 * 이 경로를 inline 으로 서빙해도 저장형 XSS 위험이 없다.
 */
export async function getOrCreateThumbnail(
  ...
  try {
    ...
    await sharp(sourcePath)
      .rotate() // EXIF 방향 반영 — 세로로 찍은 사진이 눕지 않게
    ...
    // 이미지가 아니거나 손상된 파일 — 호출부가 404로 응답한다.
