// 사용자별 알림 종류 on/off. 판정은 notification.service 에서만 한다.
...
export const notificationSettingService = {
  ...
  /** 이 종류의 알림을 보내도 되는지. 설정을 못 읽으면 보내는 쪽으로 기운다. */
  ...
  /** 여러 사람이 이 종류의 알림을 받는지 질의 한 번으로 확인한다. */
  ...
  async setMany(
    ...
    // 먼저 전부 검사하고, 하나라도 걸리면 아무것도 바꾸지 않는다(부분 반영 방지).
