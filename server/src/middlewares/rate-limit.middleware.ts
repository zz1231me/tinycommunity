...
// 메시지 발송 전용 제한 — 사용자 기준으로 센다.
// IP 기준이면 같은 사무실에서 여러 명이 대화할 때 서로의 몫을 깎아먹는다.
export const messageLimiter = rateLimit({
  windowMs: RATE_LIMIT.MESSAGE_WINDOW_MS,
  max: RATE_LIMIT.MESSAGE_MAX,
  standardHeaders: 'draft-6',
  legacyHeaders: false,
  keyGenerator: req => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const userId = (req as any).user?.id;
    return userId ? `message:user:${userId}` : `message:ip:${ipKeyGenerator(req.ip ?? '')}`;
  },
  handler: (req, res) => {
    logWarning('메시지 대량 발송 시도', { ip: req.ip });
    res.status(429).json({
      success: false,
      message: '메시지를 너무 많이 보냈습니다. 잠시 후 다시 시도해주세요.',
    });
  },
});
