  ...
  return async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      req.board = {
        ...
        permissions,
        // 권한 판정이 이미 BoardManager 를 봤으므로 그대로 실어 보낸다.
        // 핸들러가 다시 조회하지 않게 하기 위한 것이다.
...
// req.board 의 타입은 types/auth-request.ts 의 AuthRequest 한 곳에만 둔다.
// 여기서 Express.Request 를 전역으로 다시 넓히면 두 선언이 어긋나는 순간
// 컴파일이 깨진다.
