...
async function extractBundle(
  ...
  for (const entry of zip.getEntries()) {
    ...
    if (!rel || rel === '.') continue;
    // 크로스플랫폼(Windows) 안전성 — 금지 문자/예약명/ADS 차단
    if (isUnsafeBundlePath(rel)) {
      throw new BundleError(`사용할 수 없는 파일명이 있습니다: ${path.basename(rel)}`);
    }
    ...
    // 압축폭탄 방지: 실제 해제(getData) 전에 헤더의 해제 크기로 상한 초과를 먼저 차단(OOM 방지)
    const declaredSize = entry.header?.size ?? 0;
    if (totalSize + declaredSize > BUNDLE_MAX_TOTAL) {
      throw new BundleError('압축 해제 용량이 너무 큽니다.');
    }
...
export const getPageBySlug = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    sendSuccess(res, {
      ...
      title: page.title,
      html: page.html,
      externalUrl: page.externalUrl, // 있으면 클라이언트가 이 URL을 iframe src로 사용(html/번들보다 우선)
...
function validatePayload(
  res: Response,
  body: { slug?: unknown; title?: unknown; html?: unknown; externalUrl?: unknown }
): { slug: string; title: string; html: string; externalUrl: string | null } | null {
  ...
  // CustomPage.title 컬럼이 STRING(150) 이다
  if (title.length > 150) {
    sendValidationError(res, 'title', '제목은 150자를 초과할 수 없습니다.');
    return null;
  }
  ...
  // 외부 URL은 선택 — 값이 있으면 http(s)만 허용. 빈 값/미전송이면 null(URL 페이지 아님).
  ...
  if (rawUrl !== undefined && rawUrl !== null && String(rawUrl).trim() !== '') {
    ...
    if (!externalUrl) {
      sendValidationError(
        res,
        'externalUrl',
        'http:// 또는 https:// 로 시작하는 올바른 URL을 입력해주세요.'
      );
      return null;
    }
  }
...
function readMode(
  res: Response,
  raw: unknown
): { ok: true; mode: PageMode | null } | { ok: false } {
...
function readOrder(
  res: Response,
  raw: unknown
): { ok: true; order: number | null } | { ok: false } {
  ...
  if (!Number.isInteger(n) || n < ORDER_MIN || n > ORDER_MAX) {
    sendValidationError(
      res,
      'order',
      `정렬 순서는 ${ORDER_MIN}~${ORDER_MAX} 사이의 정수여야 합니다.`
    );
    return { ok: false };
  }
