...
export const deletePage = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const deleted = await wikiService.deletePage(req.params.slug);

    // 되돌릴 수 없는 삭제라 누가 무엇을 지웠는지 남긴다. 기록 실패가 삭제를 되돌리지는
    // 않으므로 기다리지 않는다(다른 감사 기록과 같은 방식).
    auditLogService
      .createAuditLog({
        actorId: userId,
      ...
      .catch(err => logError('위키 삭제 감사 기록 실패', err));
