 ...
 *
 * 숨기기는 그동안 정말로 누를 수 없지만, 개수와 별개로 아래 HIDE_TOTAL_MAX_SECONDS 가
 * 합계를 막는다 — 그래서 이 수를 올려도 '퇴근을 아예 못 하는 시간' 은 늘지 않는다.
 * 방해·문제 내기는 끝내 누를 수 있으므로 줄이 길어져도 성가실 뿐이다.
 * 퇴근 기록 시각은 어느 쪽이든 실제로 누른 순간 그대로다.
 */
