...
- **관리자**: 기능 스위치 24종(기능별 on/off, 의존성 자동 해소), 사용자·게시판·역할·태그·이벤트 관리, 테마(브랜드 색 계단 자동 생성), 보안/에러/감사/로그인 로그, 신고·IP 규칙·Rate Limit·사이트 설정
...
**Backend** — Express 5, TypeScript 6, Sequelize 6, jsonwebtoken 9, bcryptjs 3, Speakeasy 2(2FA), Winston 3, Multer 2, Sharp(아바타·썸네일), Zod 4, Helmet 8, Jest 30
...
| 운영·보안          | `SiteSettings` `FeatureFlag` `RateLimitSettings` `IpRule` `SecurityLog` `ErrorLog` `AuditLog` |
...
**스키마 변경 방식** — `sync({alter:false})` + 부팅 시 컬럼 보강입니다. 컬럼과 인덱스 추가는 자동으로
따라가지만 타입·제약 변경은 따라가지 않습니다(값이 늘어나는 ENUM 컬럼은 부팅 시 문자열로 넓힙니다).
운영 DB에서 타입을 바꿔야 할 때는 별도 마이그레이션이 필요합니다.
